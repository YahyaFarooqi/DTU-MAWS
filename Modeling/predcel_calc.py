...
class Options:
    ...
    def __init__(self, config_dict):
        ...
        # [cfu/ml] starting concentration of E. coli in new lagoon
        self.ceu0 = config_dict['ceu0']
        # [pfu/ml] concentration of phage that is transferred to the very first lagoon
        self.cp0 = config_dict['cp0']
        self.k = config_dict['k']  # binding constant of phage to E. coli
        # [min] doubling time of uninfected E. coli
        self.tu = config_dict['tu']
        self.ti = config_dict['ti']  # [min] doubling time of infected E. coli
        # [min] doubling time of phage producing E. coli
        self.tp = config_dict['tp']
        # [pfu/min] maximum production rate of phage from infected E. coli
        self.mumax = config_dict['mumax']
        # [min] time until phage production starts
        self.tpp = config_dict['tpp']
        self.f0 = config_dict['f0']  # fitness of phage
        # [cfu] maximum phage titer for meta data evaluation
        self.max_cp = config_dict['max_cp']
        # [cfu] minimum phage titer for meta data evaluation
        self.min_cp = config_dict['min_cp']
        ...
        # an epoch is the time between two transfers
        self.epochs = config_dict['epochs']
        # [cfu/ml] maximum concentration E.coli can reach under the conditions
        self.capacity = config_dict['capacity']
        self.dt = self.tl / self.tsteps

        self.growth_mode = config_dict['growth_mode']  # exp or logistic
        # Bool that decides, if initial concentrations
        self.phageonly = config_dict['phageonly']
        # float that scales the noise added to parameters at each call
        self.noisy = config_dict['noisy']
        # either lin or const or linear, dist
        self.fitnessmode = config_dict['fitnessmode']
        # number of different f-values that are possible
        self.f_prec = config_dict['f_prec']
        # share of share of fitness that is mutated
        self.to_mutate = config_dict['to_mutate']
        # sigma for gaussian mutation in mutation
        self.sigma = config_dict['sigma']
        # distribution for mutatino, either norm or skew
        self.mutation_dist = config_dict['mutation_dist']
        # parameter of skewed normal distribution
        self.skewness = config_dict['skewness']
        # wether or not to plot concentrations for each fitness
        self.plot_dist = config_dict['plot_dist']
        try:
            # Percentage of phage that dies per timestep
            self.phage_deg = config_dict['phage_degradation']
...
def e_growth_rate(current_concentration, td, o, v):
    ...
    if o.growth_mode == 'exp':
        ...
        return (log(2) / td) * current_concentration * \
            ((o.capacity - (v.cep[-1] + v.cei[-1] + v.ceu[-1])) / o.capacity)
    else:
...
def setup(o, v, logfile):
    ...
    logfile.write(
        'Calculating setup with f0 = {}, vt = {}, tl = {}\n'.format(
            o.f0, o.vt, o.tl))
    logfile.flush()
    ...
    for epoch in range(o.epochs):
        for _ in range(int(o.tsteps)):
            ...
            v.cp.append(
                d((1 - o.phage_deg) * (v.cp[v.ts - 1] + v.sdcp[v.ts] * o.dt)))
            logfile.write('{}; {}; {}\n'.format(
                v.time[-1], v.cp[-1], (v.ceu[-1] + v.cei[-1] + v.cep[-1])))

        print(('Epoch {} of {}, {}%.'.format(
            epoch, o.epochs, int(100 * epoch / o.epochs))))
...
def meta(config_dict, delta, num_datapoints, logfile):
    ...
    for fr in range(num_datapoints):  # f0
        f0 = start_config_dict['f0'] * (1 - delta) + fr * \
            start_config_dict['f0'] * (2 * delta / (num_datapoints - 1))
        config_dict['f0'] = f0
        data[f0] = {}

        for tr in range(num_datapoints):  # tl
            tl = start_config_dict['tl'] * (1 - delta) + tr * start_config_dict['tl'] * (
                2 * delta / (num_datapoints - 1))
            config_dict['tl'] = tl
            data[f0][tl] = {}

            for vr in range(num_datapoints):  # vt
                vt = start_config_dict['vt'] * (1 - delta) + vr * start_config_dict['vt'] * (
                    2 * delta / (num_datapoints - 1))
                config_dict['vt'] = vt
                ...
                count += 1
                print(('{} % ready.'.format(
                    count / (num_datapoints * num_datapoints * num_datapoints))))
    ...
    for key1 in list(data.keys()):
        ...
        for key2 in list(data[key1].keys()):
            ...
            for key3 in list(data[key1][key2].keys()):
                ...
                # overwrite data still left from data
                reduced[key1][key2][key3] = {}
                reduced[key1][key2][key3]['cpend'] = data[key1][key2][key3].cp[-2]
                valid_epochs, tendency = valid_phage_titer(
                    data[key1][key2][key3].cp[:-1], o)
...
def dist_dcp(ts, o, v):
    ...
    # mutation(v.dist_f[-1], o) to disable selection
    current_f_dist = mutation(current_f_dist, o)
