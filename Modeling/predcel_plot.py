    ...
    for key1 in list(data.keys()):
        ...
        for key2 in list(data[key1].keys()):
            ...
            for key3 in list(data[key1][key2].keys()):
                ...
                # true, when min_cp > titer in the end
...
def plot(o, v, summariesdir, prefix="", suffix=""):
    # direct plot
    all_ecoli = (np.asarray(
        v.cep[:-1]) + np.asarray(v.cei[:-1]) + np.asarray(v.ceu[:-1])).tolist()

    draw(v.time[:-1], v.cei[:-1], v.ceu[:-1], v.cep[:-1], all_ecoli,
         v.cp[:-1], True, 'predcel', prefix, suffix, summariesdir)
    draw(v.time[:-1], v.cei[:-1], v.ceu[:-1], v.cep[:-1], all_ecoli,
         v.cp[:-1], False, 'predcel', prefix, suffix, summariesdir)

    # derivatives
    all_ecoli = (np.asarray(
        v.sdcep[:-1]) + np.asarray(v.sdcei[:-1]) + np.asarray(v.sdceu[:-1])).tolist()
    draw(v.time[:-1],
         v.sdcei[:-1],
         v.sdceu[:-1],
         v.sdcep[:-1],
    ...
    if o.fitnessmode == 'dist':
        ...
        drawdist(v.time[:-1], True, 'predcel',
                 prefix, suffix, o, v, summariesdir)
        drawdist(v.time[:-1], False, 'predcel',
                 prefix, suffix, o, v, summariesdir)
...
def drawdist(x, log, name, prefix, suffix, o, v, summariesdir):
    ...
    if o.plot_dist == "True":
        for f_val in list(v.dist_f[0].keys()):
            cepf = np.asarray([v.dist_f[t][f_val] for t in range(
                len(v.dist_f) - 1)]) * np.asarray(v.cep[:-1])
            try:
                p1 = p1 + ax.plot(x,
                                  d(cepf),
                                  label='cep(f={})'.format(f_val),
                                  color=list(colors.values())[col])
            except BaseException:
                p1 = ax.plot(x, d(cepf), label='cep(f={})'.format(
                    f_val), color=list(colors.values())[col])
            col += 1
...
def plot_meta(reduced, maxepochs, summariesdir, prefix='', suffix='', vl=1):
    ...
    for a in range(scoret.shape[0]):
        for x in range(scoret.shape[1]):
            for y in range(scoret.shape[2]):
                scoret[a, x, y] = coltransform(
                    reduced[akeys[a]][xkeys[x]][ykeys[y]], maxepochs)
    ...
    draw_meta(
        ...
        prefix,
        suffix + 'f',
        'Time in one lagoon [min]',
        'Transfer volume/lagoon volume',
        'Search for tl and vt  with f',
        summariesdir)

    for a in range(scoret.shape[1]):
        for x in range(scoret.shape[0]):
            for y in range(scoret.shape[2]):
                scoret[x, a, y] = coltransform(
                    reduced[akeys[a]][xkeys[x]][ykeys[y]], maxepochs)
    ...
    draw_meta(
        ...
        'Initial fitness',
        'Time in one lagoon [min]',
        'Search for f0 and tl  with vt/vl',
        summariesdir)
...
def draw_meta(
    ...
    for fdraw in range(scoret.shape[0]):
        ...
        pcm = ax.imshow(scoret[fdraw, :, :], origin='lower',
                        ...
                        cmap=ccmap,
                        clim=[-1, 1])
        clb = fig.colorbar(pcm, ax=ax, extend='both', ticks=np.asarray(
...
def plot_dist_f(v, o, prefix, suffix, summariesdir):
    ...
    plt.savefig(
        os.path.join(
            summariesdir,
            "{}fitness_distribution_{}.png".format(
                prefix,
                suffix)))
    plt.close()
