...
def plot(data, data_2, seq_count, file, original):
    ...
    for idx in mutated:
        amount_of_total_mutations = data_2['{}{}'.format(
            original[idx], idx + 1)] / sum_total_muts
        file.write(
            '\n\nPosition: {}\nOriginal: {}\nAmount of total mutations: {}\n'.format(
                idx + 1,
                original[idx],
                amount_of_total_mutations))
        amount = {}
        for el in data[idx]:
...
logfile = open(ldir, 'w')
logfile.write(
    'Starting with read-directory "{}", {} generations.\n'.format(rdir, until))
logfile.flush()
