...
from . import customlayers as customlayers
...
class DeeProtein():
    ...
    def save_model(self, network, session, step, name='DeeProtein'):
        ...
        param_save_dir = os.path.join(self._opts._summariesdir,
                                      'checkpoint_saves/')
        conv_vars = [
            var for var in network.all_params if 'dense' and 'outlayer' not in var.name]

        lower_enc = [
            var for var in network.all_params if 'lower_encoder' in var.name]
        ...
        if name == 'Encoder':
            tl.files.save_npz_dict(
                lower_enc, name=os.path.join(
                    param_save_dir, '%s_lower_enc_part.npz' %
                    name), sess=session)
    ...
    def input_pipeline(self, file_paths, valid_mode=False):
        ...
        with tf.name_scope('input_pipeline_%s' % id):
            ...
            batch, labels = tf.train.shuffle_batch([seq_tensor, label],
                                                   ...
                                                   # should this match the
                                                   # filesize of example files?
...
##########################################################################
    ...
    def feed_dict(self):
        ...
        if self.is_train:
            feed_dict = {
                # self.training_bool: self.is_train, #the training bool decides
                # which queue we use and if droppout is applied or not.
            }
    ...
    def initialize_helpers(self):
        ...
        self.batchgen = helpers.BatchGenerator(
            self._opts, self._kmer2vec_embedding, self._kmer2id)
    ...
    def eval_while_train(self, step=1, eval_steps=200):
        ...
        with eval_graph.as_default():
            # TODO fix this as gpu0 is always requested.
            with tf.device('/gpu:{}'.format(self._opts._gpu)):
                with tf.Session(config=config) as sess:
                    # first time we validate TODO: implement this smarter...
    ...
    def infer_single(self, seq):
        ...
        with tf.Graph().as_default():
            ...
            input_seq_node = tf.placeholder(
                [self._opts._batchsize, self._opts._depth, self._opts._windowlength, 1])
            ...
            logits = self.session.run(
                inf_sigmoid_logits, feed_dict={
                    input_seq_node: encoded_seq})
            ...
            predicted_classes = [self.batchgen.id_to_class[_id]
        ...
        with tf.Graph().as_default():
            ...
            for i in range(
                ...
                embed_batch = self.session.run(
                    self.graph_no_dense.outputs, feed_dict={
        ...
        with g.as_default():
            ...
            with tf.Session() as embedding_sess:
                ...
                _ = embedding_sess.run(
                    mockOP, feed_dict={
                        embedding_node: fitted_embedding})  # TODO edit here
        ...
        with tf.Graph().as_default():
            ...
            self.input_seq_node = tf.placeholder(
                shape=[
                    ...
                    1],
                dtype=tf.float32)
            inference_net, inference_garbage_detec = self.model(
                self.input_seq_node, valid_mode=False)

            double_sigmoid_logits = tf.nn.sigmoid(inference_net.outputs)

            self.mean_inf_sigmoid_logits, self.var_inf_sigmoid_logits = tf.nn.moments(
                double_sigmoid_logits, axes=[2])

            self.inf_garbage_logit = tf.nn.softmax(
                inference_garbage_detec.outputs, dim=1)

            self.session.run(tf.global_variables_initializer())
            # restore all the weight
            _ = self.load_model_weights(
                inference_net,
                session=self.session,
                name='Classifier')  # THIS RUNS THE SESSION INTERNALLY
