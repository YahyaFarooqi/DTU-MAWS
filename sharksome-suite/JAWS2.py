...
def consistendBonds(structures):
    ...
    consistentBonds = []
    allBonds = [[[ttp, tfp]
                 for ttp, tfp in zip(*bonds(struc))] for struc in structures]
    for idx, bonds in enumerate(allBonds):
        for bond in bonds:
            for bonds in allBonds[:idx] + allBonds[idx + 1:]:
                if bond in bonds and bond not in consistentBonds:
...
# Mutate a single <letter> according to an <alphabet> weighted by
# <probabilities>
...
def Mutate(
    ...
    if invariantSequence is not None:
        ...
        if pMutation == 1:
            def mutate(x): return MutateLetter(x, alphabet, probabilities)
            ...
            for idx in range(len(string)):
                if WeightedChoice([0, 1], [pNMut, pMut]) == 0:
                    ...
                    result[idx] = MutateLetter(
                        result[idx], alphabet, probabilities)
            result = ''.join(result)
    return result
...
# Generate a random string of <length> from <alphabet> weighted by
# <probabilities>
...
def PopulateRandomStrings(length, alphabet, probabilities, Nsamples):
    return [RandomString(length, alphabet, probabilities)
            for i in range(Nsamples)]

# as above, invariant strings


def PopulateInvariantStrings(
        structure,
        sequence,
        alphabet,
        probabilities,
        Nsamples):
    return [
        RandomInvariantString(
            structure,
            sequence,
            alphabet,
            probabilities) for i in range(Nsamples)]
...
# Evolve a population according to a <fitness_function> (Quasi Clonal
# Selection)
...
def EvolveStrings(
    ...
    counts = [int(math.floor(fitness[i] / (sum(fitness[0:fuzz + 1]
                                               ) + 1e-6) * Nsamples)) for i in range(fuzz + 1)]
    counts[0] += Nsamples - sum(counts)

    def mutate(x): return Mutate(x, alphabet,
                                 probabilities,
                                 invariantSequence=sequence,
                                 pMutation=pMutation[0])

    def consistentMutate(x): return ConsistentMutate(
        x,
        alphabet,
        probabilities,
        structures,
...
def ConstrainBonded(sequence, constraint):
    ...
    matrix = [[RNA.get_pr(i + 1, j + 1) for i in range(len(sequence))]
...
def ConstrainEntropy(sequence, position):
    ...
    score = 0
    for j in range(len(sequence)):
        score -= matrix[position][j] * \
            math.log((matrix[position][j] + 1e-12) * len(seq))
    return score
...
def ConstrainNonbonded(sequence, constraint):
    ...
    matrix = [[RNA.get_pr(i + 1, j + 1) for i in range(len(sequence))]
              for j in range(len(sequence))]
    score = 0
    nonbonded = [idx for idx, marker in enumerate(
        constraint) if marker in ".x"]
...
def ConstrainStructure(sequence, constraint):
    return ConstrainBonded(sequence, constraint) + \
        ConstrainNonbonded(sequence, constraint)


def ConstrainGC(sequence, GC):
    return abs(GC - sum([1. / len(sequence)
               for ntide in sequence if ntide in "GCgc"]) * 100)


def ConstrainAT(sequence, AT):
    return abs(AT - sum([1. / len(sequence)
               for ntide in sequence if ntide in "ATat"]) * 100)
...
def ConstrainSingleRepeat(sequence, maxRep, base=None):
    ...
    lastNtide = ""
    if base is None:
        def doCount(x): return True
    else:
        def doCount(x): return x == base
