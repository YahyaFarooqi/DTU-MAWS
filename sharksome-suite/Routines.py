...
def kullbackLeiblerDivergence(sample, reference_sample):
    return -math.fsum(np.asarray(sample) * list(map(math.log,
                                                    np.asarray(sample) / np.asarray(reference_sample))))


def Entropy(sample):
    return -math.fsum(list(map(math.fmul,
                               np.asarray(sample),
                               list(map(math.log,
                                        list(map(math.fmul,
                                                 np.asarray(sample),
...
def ZPS(sample, beta=0.01):
    Z = math.fsum(list(map(math.exp, list(
        map(math.fmul, [-beta] * len(sample), np.asarray(sample))))))
    P = list(map(math.fdiv,
                 list(map(math.exp,
                          list(map(math.fmul,
                                   [-beta] * len(sample),
                                   np.asarray(sample))))),
