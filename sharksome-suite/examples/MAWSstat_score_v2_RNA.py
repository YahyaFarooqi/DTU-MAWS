...
for ntide in 'GAUC':
    ...
    # Build the Complex
    complex.build()
    currentCenter = centerOfMass(np.asarray(
        nostrom(complex.positions[complex.chains[1].start:])))
    shift = currentCenter * unit.angstrom - \
        centerOfMass(np.asarray(nostrom(c.positions))) * unit.angstrom
    aptamer.translate_global(shift)
    complex.positions = complex.positions[:
                                          complex.chains[1].start] + c.positions[:]
...
output.write("{0}: Completed first step. Selected nucleotide: {1}\n".format(
    str(datetime.now()), best_sequence))
...
for i in range(N_NTIDES):
    ...
    output.write("{0}: Completed step {1}. Selected sequence: {2}\n".format(
        str(datetime.now()), i + 2, best_sequence))
