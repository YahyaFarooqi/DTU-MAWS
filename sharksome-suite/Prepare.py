...
# makeLib
# parametrizes a pdb or mol2 file to generate a .lib library file for
# tleap / nab
...
def makeLib(
    ...
    if not parameterized:
        # Execute
        subprocess.call(
            "antechamber -i %s -fi %s -o %s.mol2 -fo mol2 -c %s -rn %s -at %s" %
            (file_path, extension, name, charges, residue_name, atom_type), shell=True)
        subprocess.call(
            "parmchk -i %s.mol2 -f mol2 -o %s.frcmod" %
            (name, lib_path), shell=True)
