...
def radius(center, positions):
    return max(
        list(map(np.linalg.norm, np.asarray(positions) - np.asarray(center))))


@jit
def kullbackLeiblerDivergenceKernel(sample, reference_sample):
    return -(np.array(sample) * np.log(np.array(sample) /
             np.array(reference_sample))).sum()
...
def EntropyKernel(sample):
    ...
    return -(np.array(sample) *
             np.log(catchZero(np.array(sample) * len(sample)))).sum()
