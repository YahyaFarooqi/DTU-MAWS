...
def XMLParseStructure(path):
    ...
    backbone = []
    connect = []
    residue_path = root.find("residuePath").text if root.find(
        "residuePath") else None
    ...
    for residue in root.findall("residue"):
        ...
        for rotation in residue.findall('rotation'):
            residue_rotations.append(
                [
                    residue.get("name"), int(
                        rotation.find("start").text), int(
                        rotation.find("bond").text), None if rotation.find("end").text == 'end' else int(
                        rotation.find("end").text)])
...
def XMLStructure(path):
    ...
    return Structure.Structure(*parsed[0:2],
