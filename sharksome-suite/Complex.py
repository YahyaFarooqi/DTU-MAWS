...
class Chain(object):
    ...
    def create_sequence(self, sequence):
        ...
        for letter in sequence_array:
            if letter in self.structure.residue_names:
                ...
                raise ValueError(
                    'Residue not defined! CANNOT create sequence!')
        self.alias_sequence = sequence
        self.sequence = self.structure.translate(self.alias_sequence)
    ...
    def prepend_sequence(self, sequence):
        ...
        self.prepend_history = self.sequence_array[:len(
            self.sequence_array) - seq_ar_length]
    ...
    #  @param reverse
    # specifies, whether positions IN, or OUTSIDE the element should be
    # rotated
    def rotate_element(self, element, angle, reverse=False):
        ...
        if len(revised_element) == 3 and revised_element[2] is not None:
            ...
            raise ValueError(
                'Rotable element contains too many or too few components!')
    ...
    def rotate_in_residue(
        ...
        element = self.structure.rotating_elements[self.sequence_array[revised_residue_index]
                                                   ][residue_element_index]
        # LOOK HERE!!!!!!!!!!!!!
        ...
        if element[2] is None:
            revised_element = [
                element[0] +
                self.residues_start[revised_residue_index],
                element[1] +
                self.residues_start[revised_residue_index],
                None]
        elif element[2] == 0:
    ...
    def rotate_historic_element(self, historic_element, angle):
        if historic_element[2]:
            self.rotate_element([historic_element[0] +
                                 self.start_history -
                                 self.start, historic_element[1] +
                                 self.start_history -
                                 self.start, historic_element[2] +
                                 self.start_history -
                                 self.start], angle)
        else:
            self.rotate_element([historic_element[0] +
                                 self.start_history -
                                 self.start, historic_element[0] +
                                 self.start_history -
                                 self.start, None], angle)
...
class Complex(object):
    ...
    def build(self, target_path="", file_name="out"):
        ...
        if self.chains:
            ...
            chain_string = " ".join([("CHAIN%s" % index if chain.sequence else "")
                                    for index, chain in enumerate(self.chains)])
            self.build_string += """
                                UNION = combine {%s}
            ...
            result = subprocess.call(
                "tleap -f %s%s.in" %
                (target_path, file_name), shell=True)
            self.prmtop = app.AmberPrmtopFile(
                target_path + file_name + ".prmtop")
            self.inpcrd = app.AmberInpcrdFile(
                target_path + file_name + ".inpcrd")
        ...
        for index, chain in enumerate(self.chains):
            if not (chain in exclusion):
                ...
                post_positions = self.positions[chain.start_history +
                                                chain.length_history:chain.start + chain.length]
                ...
                if len(pre_positions) != 0 and chain.prepend_history:
                    ...
                    pre_vector = self.positions[chain.start_history + chain.structure.connect[chain.prepend_history[-1]]
                                                [1][0]] - self.positions[chain.start_history + 1]
                    old_pre_vector = old_positions[chain.start] - \
                        old_positions[chain.start + 1]
                    angle = -ang(nostrom(pre_vector), nostrom(old_pre_vector))
                    axis = np.cross(
                        np.asarray(
                            nostrom(pre_vector)), np.asarray(
                            nostrom(old_pre_vector)))
                    ...
                    shift_forward = mm.Vec3(
                        0, 0, 0) * unit.angstroms - pos[-1 + chain.structure.connect[chain.prepend_history[-1]][1][0]]
                    s = np.math.sin(phi_2)
                    c = np.math.cos(phi_2)
                    rot = np.array([[2 *
                                     (np.power(x, 2) -
                    ...
                    shift_back = chain_positions[chain.structure.connect[chain.sequence_array[len(
                        ...
                        roted = np.dot(
                            np.array(
                                pos[j].value_in_unit(
                                    unit.angstrom)), rot)
                        pos[j] = mm.Vec3(
                            roted[0], roted[1], roted[2]) * unit.angstrom
                        pos[j] += shift_back + pre_bond_shift

                    pre_positions = pos[:]
                    chain_positions[0] += pre_bond_shift

                    self.positions = self.positions[:chain.start] + pre_positions[:] + \
                        chain_positions[1:] + self.positions[chain.start + chain.length:]
                ...
                if len(post_positions) != 0 and chain.append_history:
                    ...
                    post_positions = self.positions[chain.start_history +
                                                    chain.length_history - 1:chain.start_history + chain.length]
                    post_vector = self.positions[chain.start_history + chain.length_history -
                                                 1] - self.positions[chain.start_history + chain.length_history - 2]
                    old_post_vector = old_positions[chain.start_history + chain.length_history -
                                                    1] - old_positions[chain.start_history + chain.length_history - 2]
                    angle = -ang(nostrom(post_vector),
                                 nostrom(old_post_vector))
                    axis = np.cross(
                        np.asarray(
                            nostrom(post_vector)), np.asarray(
                            nostrom(old_post_vector)))
                    if all(axis == np.zeros(3)):
                        axis = np.array([1., 0., 0.])
                        angle = 0.
                    ...
                    pos = post_positions[:]
                    shift_forward = mm.Vec3(
                        0, 0, 0) * unit.angstroms - pos[chain.structure.connect[chain.append_history[0]][0][0]]
                    s = np.math.sin(phi_2)
                    c = np.math.cos(phi_2)
                    rot = np.array([[2 *
                                     (np.power(x, 2) -
                    ...
                    shift_back = chain_positions[chain.structure.connect[chain.sequence_array[-len(
                        chain.append_history)]][0][1]]

                    for pos_idx, pos_elem in enumerate(pos):
                        roted = np.dot(
                            np.array(
                                pos_elem.value_in_unit(
                                    unit.angstrom)), rot)
                        pos[pos_idx] = mm.Vec3(
                            roted[0], roted[1], roted[2]) * unit.angstrom
                    ...
                    self.positions = self.positions[:chain.start] + chain_positions[:-1] + \
                        post_positions[:] + self.positions[chain.start + chain.length:]

                    # Stop fixing positions of propended atoms.

                if not (chain.append_history or chain.prepend_history):
                    self.positions = self.positions[:chain.start] + old_positions[chain.start_history:chain.start_history +
                                                                                  chain.length_history] + self.positions[chain.start + chain.length:]
    ...
    def rotate_global(self, element, axis, angle, reverse=False, glob=True):
        if self.positions:
            ...
            # print(element)
            if reverse:
                shift_forward = mm.Vec3(0, 0, 0) * \
                    unit.angstroms - pos[element[2]]
            else:
                shift_forward = mm.Vec3(
    ...
    def fit_sequence_to_chain(self, chain_id, sequence):
        ...
        self.translate_global(
            chain.element, current_positions[current_chain_start] - self.positions[chain.start])
        vec0 = self.positions[chain.element[1]] - \
            self.positions[chain.element[0]]
        vec1 = current_positions[current_chain_element[1]
                                 ] - current_positions[current_chain_element[0]]
        angle = ang(nostrom(vec0), nostrom(vec1))
        global_axis = mm.Vec3(
            *np.cross(np.asarray(nostrom(vec0)), np.asarray(nostrom(vec1)))) * unit.angstrom
        ...
        for residue_id, [residue, residue_current] in enumerate(
            ...
            for element_id, (element, element_current) in enumerate(zip(
                    chain.structure.rotating_elements[residue], chain.structure.rotating_elements[residue_current])):
                # print(element)
                revised0 = element[0] + chain.start + \
                    chain.residues_start[residue_id]
                revised1 = element[1] + chain.start + \
                    chain.residues_start[residue_id]
                revised0_current = element_current[0] + \
                    current_chain_start + current_residues_start[residue_id]
                revised1_current = element_current[1] + \
                    current_chain_start + current_residues_start[residue_id]
                ...
                if not np.linalg.norm(np.asarray(nostrom(axis))) == 0:
                    ...
                    axis = mm.Vec3(1., 0., 0.)
                # Bond to next-to-nearest neighbour along increasing indices in
                # the element
                vec0 = self.positions[revised1 + 1] - self.positions[revised1]
                vec1 = current_positions[revised1_current +
                                         1] - current_positions[revised1_current]
                # Projection onto plane perpendicular to axis
                proj_vec0 = np.asarray(nostrom(vec0)) - \
                    np.dot(axis, nostrom(vec0)) * nostrom(axis)
                proj_vec1 = np.asarray(nostrom(vec1)) - \
                    np.dot(axis, nostrom(vec1)) * nostrom(axis)
    ...
    def fit_sequence_to_chain_split_join(self, chain_id, sequence):
        ...
        self.rebuild(exclusion=[chain])
        # Split Positions
        current_positions_split = [current_positions[chain.structure.backbone_elements[residue][idx][0] +
                                                     current_residues_start[idy]: chain.structure.backbone_elements[residue][idx][1] +
                                                     current_residues_start[idy] +
                                                     2 -
                                                     idx] for idy, residue in enumerate(current_sequence_array) for idx in [0, 1]]

        positions_split = [self.positions[chain.structure.backbone_elements[residue][0][1] +
                                          chain.residues_start[idy]: chain.structure.backbone_elements[residue][1][0] +
                                          chain.residues_start[idy]] for idy, residue in enumerate(new_sequence_array)]

        positions = []
        for index, residue in enumerate(chain.sequence_array):
            #substituent -= substituent[0]
            subst = positions_split[index]
            vec0 = subst[chain.structure.backbone_elements[residue][0][2] -
                         chain.structure.backbone_elements[residue][0][1]] - subst[0]
            vec1 = current_positions[current_residues_start[index] + chain.structure.backbone_elements[current_sequence_array[index]][0][2]] - \
                current_positions[current_residues_start[index] + chain.structure.backbone_elements[current_sequence_array[index]][0][1]]  # - current_positions_split[2*index][-2]
            ...
            shift_forward = subst[chain.structure.backbone_elements[residue][
                0][2] - chain.structure.backbone_elements[residue][0][1]]
            for j in range(
                ...
                roted = np.dot(
                    np.array(
                        subst[j].value_in_unit(
                            unit.angstrom)),
                    rot)
                subst[j] = mm.Vec3(
                    roted[0], roted[1], roted[2]) * unit.angstrom
                subst[j] += current_positions[current_residues_start[index] +
                                              chain.structure.backbone_elements[current_sequence_array[index]][0][2]]
            ...
            for chain in self.chains:
                for idx, residue in enumerate(chain.sequence_array):
                    for j in range(max_step_iterations):
                        ...
                        chain.rotate_in_residue(idx, np.random.choice([elem for elem in range(len(
                            chain.structure.rotating_elements[residue]))]), np.random.uniform(-np.math.pi, np.math.pi))
    ...
    def pert_min(self, size=1e-1, iterations=50):
        for repeat in range(iterations):
            for i in range(len(self.positions)):
                self.positions[i] += np.random.uniform(-size,
                                                       size, 3) * unit.angstrom
            self.minimize()
