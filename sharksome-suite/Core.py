...
class Box(Space):
    ...
    def __init__(
        ...
        def boolean(position):
            x_in = (
                position[0] <= centre[0] +
                x_width /
                2 and position[0] >= centre[0] -
                x_width /
                2)
            y_in = (
                position[1] <= centre[1] +
                y_width /
                2 and position[1] >= centre[1] -
                y_width /
                2)
            z_in = (
                position[2] <= centre[2] +
                z_width /
                2 and position[2] >= centre[2] -
                z_width /
                2)
    ...
    def generator(self):
        ...
        return np.array([np.random.uniform(self.centre[0] -
                                           self.x_width /
                                           2, self.centre[0] +
                                           self.x_width /
                                           2), np.random.uniform(self.centre[1] -
                                                                 self.y_width /
                                                                 2, self.centre[1] +
                                                                 self.y_width /
                                                                 2), np.random.uniform(self.centre[2] -
                                                                                       self.z_width /
                                                                                       2, self.centre[2] +
                                                                                       self.z_width /
                                                                                       2), x, y, z, rotation_angle])
# @class Cube
...
class Sampler(object):
    ...
    # References a function returning an object, which is appended to the
    # .samples List.
...
class EnergySampler(Sampler):
    ...
    def transform_complex(transformation_array):
        ...
        aptamer_position_dummy = np.tensordot(
            rotation_matrix, aptamer_position_dummy, axis=0)
...
class EnergyGridSampler(EnergySampler):
    ...
    def __init__(self, Complex, space, size,
                 grid_spacing=1, grid_bounds=[-1e2, 1e2]):
        ...
        self.grid = cartesian(
            (np.linspace(
                self.grid_bounds[0], self.grid_bounds[1], grid_spacing), np.linspace(
                self.grid_bounds[0], self.grid_bounds[1], grid_spacing), np.linspace(
                self.grid_bounds[0], self.grid_bounds[1], grid_spacing)))
...
class Structure(object):
    ...
    def __init__(self, residue_names, residue_length=None,
        ...
        self.residue_bond_lengths = defaultdict(
            lambda: [1.0 * unit.angstrom, 1.0 * unit.angstrom])
    ...
    def add_rotation(self, residue_name, rotations):
        ...
        if isinstance(rotations[0], str):
            ...
            raise ValueError(
                "The input supplied is not a three component list specifying a rotation!")
    ...
    def delete_rotation(self, residue_name, rotations):
        if isinstance(rotations[0], str):
            ...
            raise ValueError(
                "The input supplied is not a two component list specifying a rotation!")
        return self.rotating_elements
    ...
    def translate(self, sequence):
        ...
        if len(sequence_array) == 1:
            ...
            return " ".join([self.alias[sequence_array[0]][1]] + [aliasElement[2] for aliasElement in map(
                self.alias.__getitem__, sequence_array)] + [self.alias[sequence_array[-1]][3]])
...
class Complex(object):
    ...
    class Chain(object):
        ...
        def rotate_element(self, element, angle):
            ...
            elif (len(element) == 3) and (element[2] is None):
                revised_element = [
                    element[0] + self.start,
                    element[1] + self.start,
        ...
        def rotate_in_residue(
                ...
                angle):
            element = self.structure.rotating_elements[self.sequence_array[residue_index]
                                                       ][residue_element_index]
            # LOOK HERE!!!!!!!!!!!!!
            if element[1] < 0:
            ...
            if element[2] is None:
                revised_element = [
                    element[0] +
                    self.start +
                    self.residues_start[residue_index],
                    element[1] +
                    self.start +
                    self.residues_start[residue_index],
                    None]
                ...
                revised_element = [
                    element[0] + self.start + self.residues_start[residue_index],
        ...
        def rotate_in_historic_residue(
                self, historic_index, element_index, angle):
            offset = len(self.prepend_history)
            self.rotate_in_residue(
                historic_index + offset, element_index, angle)
    ...
    def build(self, target_path="", file_name="out"):
        ...
        if self.chains:
            ...
            chain_string = " ".join(["CHAIN%s" %
                                     index for index in range(len(self.chains))])
            self.build_string += """
                                UNION = combine {%s}
        ...
        for chain in self.chains:
            if not (chain in exclusion):
                ...
                post_positions = self.positions[chain.start_history +
                                                chain.length_history:chain.length]
                ...
                if pre_positions:
                    ...
                    pre_bond_shift = (
                        chain.structure.connect[chain.prepend_history[-1]][2] - 1.) * old_pre_vector
                    ...
                    self.positions = pre_positions[:] + \
                        chain_positions[1:] + post_positions[1:]
                ...
                if post_positions:
                    ...
                    post_positions = self.positions[chain.start_history +
                                                    chain.length_history - 1:chain.length]
                    ...
                    post_bond_shift = (
                        chain.structure.connect[chain.append_history[0]][2] - 1.) * old_post_vector
                    shift_back = chain_positions[chain.structure.connect[chain.sequence_array[-len(
    ...
    def fit_sequence_to_chain(self, chain_id, sequence):
        ...
        self.translate_global(
            chain.element, current_positions[chain.start] - self.positions[chain.start])
        vec0 = self.positions[chain.element[1]] - \
            self.positions[chain.element[0]]
        vec1 = self.current_positions[chain.element[1]
                                      ] - self.current_positions[chain.element[0]]
        angle = ang(vec0, vec1)
        ...
        for residue_id, residue in enumerate(chain.sequence_array):
            for element_id, element in enumerate(
                    chain.structure.rotating_elements[residue]):
                revised0 = element[0] + chain.start + \
                    chain.residues_start[residue_id]
                revised1 = element[1] + chain.start + \
                    chain.residues_start[residue_id]
                axis = current_positions[revised1] - \
                    current_positions[revised0]
                ...
                vec1 = self.current_positions[revised1 +
                                              1] - self.current_positions[revised1]
    ...
    def minimize(self, max_iterations=500):
        ...
        self.positions = self.simulation.context.getState(
            getPositions=True, getEnergy=True, groups=1).getPositions()
        free_E = state.getPotentialEnergy().value_in_unit(unit.kilojoules_per_mole)
        return free_E
...
# Turn computation / sampler into functions? They are not very class-like,
# and functions would be easier to MPIify
...
class Computation(object):
    ...
    # naively compute the partition function Z, probabilities P and entropy S
    # of a given array of energies
    def naiveZPS(self):
