...
#N_CHUNKS = 1
NKEEP = 2
# Number of rotable junctions in DNA, to distinguish forward and backward
# rotation
N_ELEMENTS = 4
...
output.write(
    "Number of further steps: {0} (sequence length = )\n".format(
        N_NTIDES,
        N_NTIDES +
        1))
...
# Create a sampling Cube of Diameter 50. Angstroms around the pdb center
# of mass
...
for ntide in 'GAUC':
    ...
    for i in range(FIRST_CHUNK_SIZE):
        ...
        # Translate the aptamer by the displacement part of the sample from the
        # cube generator
        aptamer.translate_global(orientation[0:3] * unit.angstrom)
        # Rotate the aptamer by the rotation part of the sample from the cube
        # generator
    ...
    entropyLog.write(
        "SEQUENCE: {0} ENTROPY: {1} ENERGY: {2}\n".format(
            aptamer.alias_sequence, entropy, free_E))
    # Check if best ...
    if len(best_entropies) < NKEEP:
        ...
        worst_entropy_index = min(
            enumerate(best_entropies),
            key=itemgetter(1))[0]
...
for idx in range(NKEEP):
    ...
    # Output best as well
    pdblog = open(
        "{0}_best_cand{2}_1_{1}.pdb".format(
            JOB_NAME, ntide, idx), "w")
    app.PDBFile.writeModel(
        best_topologies[idx],
        best_positions[idx],
        file=pdblog,
...
for i in range(N_NTIDES):
    ...
    for candidate in range(NKEEP):
        ...
        for ntide in 'GAUC':
            ...
            for append in [True, False]:
                ...
                # outputs
                pdblog = open(
                    "{0}_{1}_{2}.pdb".format(
                        JOB_NAME, i + 2, ntide), "w")
                app.PDBFile.writeModel(
                    copy.deepcopy(
                        complex.topology),
                    position[:],
                ...
                if len(best_entropies) < NKEEP:
                    ...
                    worst_entropy_index = min(
                        enumerate(best_entropies), key=itemgetter(1))[0]
                    best_entropies[worst_entropy_index] = entropy
                    best_sequences[worst_entropy_index] = aptamer.alias_sequence
                    best_positions[worst_entropy_index] = position[:]
                    best_topologies[worst_entropy_index] = copy.deepcopy(
                        complex.topology)
    ...
    for idx in range(NKEEP):
        ...
        output.write("{0}: Completed step {1}. Selected sequence: {2}\n".format(
            str(datetime.now()), i + 2, best_sequences[idx]))
        pdblog = open(
            "{0}_best_cand{3}_{1}_{2}.pdb".format(
                JOB_NAME, i + 2, ntide, idx), "w")
        app.PDBFile.writeModel(
            best_topologies[idx],
            best_positions[idx],
            file=pdblog,
...
for idx in range(NKEEP):
    ...
    pdb_result = open(
        "{0}_RESULT_CANDIDATE_{1}.pdb".format(
            JOB_NAME, idx), "w")
    app.PDBFile.writeModel(
